...
img=imread('C:\Users\Gökhan Yazýcý\Desktop\test\forged2_gj90.png'); % Ýnput file
Shift_Count=20;
Start_Time=tic;%Geçen süreyi hesaplar. %Evaluates time.
gray_img=img(:,:,1)*0.299+img(:,:,2)*0.587+img(:,:,3)*0.114; % griye çevirir. %Changing image to gray.
[M N]=size(gray_img); % Taking row and column information about input!
B=16; %Blok boyutu=B x B  %blok size
MinShiftSize=B;        
num_blocks=(M-B+1)*(N-B+1);%blok sayýsý &block number
...
%         title(sprintf('Shift Index=%d   Shift Vector=( %d , %d )',i,ShiftVectors(i,1),ShiftVectors(i,2)));
        ...
        maskeleme = imread('forged2_maske.png'); % Taking Mask for measurement.
        test = getFmeasure(image_mask,maskeleme)  %Testing algorithm using Measurement function. First input is our input, second is mask.
    else
...
title('Sonuç');
for i=1:size(blockShiftIndex)%Until size of equal blocks
                %Coloring the blocks
